 ...
 */
public final class XPropertiesGenerator
{
    ...
    static
    {
        FORCED_LINE_SEP[XRBEPreferences.NEW_LINE_UNIX] = "\\\\n";
        FORCED_LINE_SEP[XRBEPreferences.NEW_LINE_WIN] = "\\\\r\\\\n";
        FORCED_LINE_SEP[XRBEPreferences.NEW_LINE_MAC] = "\\\\r";
    }

    /**
     * Constructor.
     */
    private XPropertiesGenerator()
    {
        super();
    }
    ...
    public static String generate(Bundle bundle)
    {
        ...
        int numOfLineBreaks = XRBEPreferences.getGroupLineBreaks();
        ...
        {
            if (XRBEPreferences.getShowGenerator() && !headComment.startsWith(GENERATED_BY))
            {
        ...
        for (Iterator<String> iter = bundle.getKeys().iterator(); iter.hasNext();)
        {
            ...
            {
                // escape backslashes
                if (XRBEPreferences.getConvertUnicodeToEncoded())
                {
                    value = value.replaceAll("\\\\", "\\\\\\\\");
                }

                // handle new lines in value
                if (XRBEPreferences.getForceNewLineType())
                {
                    value = value.replaceAll("\r\n|\r|\n", FORCED_LINE_SEP[XRBEPreferences.getNewLineType()]);
                }
                else
                {
            ...
            if (XRBEPreferences.getKeepEmptyFields() || value.length() > 0)
            {
                // handle group equal align and line break options
                if (XRBEPreferences.getGroupKeys())
                {
                ...
                if (XRBEPreferences.getConvertUnicodeToEncoded())
                {
                    key = XPropertiesGenerator.convertUnicodeToEncoded(key);
                    value = XPropertiesGenerator.convertUnicodeToEncoded(value);
                }
    ...
    private static char toHex(int nibble)
    {
        ...
        if (!XRBEPreferences.getConvertUnicodeToEncodedUpper()) { return Character.toLowerCase(hexChar); }
        return hexChar;
    }
    ...
    private static void appendValue(StringBuffer text, String value, int equalIndex, boolean commented)
    {
        if (value != null)
        {
            ...
            int lineLength = XRBEPreferences.getWrapCharLimit() - 1;
            int valueStartPos = equalIndex;
            if (XRBEPreferences.getSpacesAroundEqualSigns())
            {
                valueStartPos += 3;
            }
            else
            ...
            if (XRBEPreferences.getNewLineNice())
            {
                value = value.replaceAll("(\\\\r\\\\n|\\\\r|\\\\n)", "$1\\\\" + SYSTEM_LINE_SEP);
            }
            // Wrap lines
            if (XRBEPreferences.getWrapLines() && valueStartPos < lineLength)
            {
                ...
                while (valueBuf.length() + valueStartPos > lineLength || valueBuf.indexOf("\n") != -1)
                {
                    ...
                    if (!XRBEPreferences.getWrapAlignEqualSigns())
                    {
                        valueStartPos = XRBEPreferences.getWrapIndentSpaces();
                    }
    ...
    {
        String sep = XRBEPreferences.getKeyGroupSeparator();
        int deepness = XRBEPreferences.getGroupLevelDepth();
        int endIndex = 0;
        int levelFound = 0;
        ...
        if (levelFound != 0)
        {
            if (levelFound < deepness) { return key; }
    ...
    {
        int equalIndex = -1;
        boolean alignEquals = XRBEPreferences.getAlignEqualSigns();
        boolean groupKeys = XRBEPreferences.getGroupKeys();
        boolean groupAlignEquals = XRBEPreferences.getGroupAlignEqualSigns();

        // Exit now if we are not aligning equals
        if (!alignEquals || groupKeys && !groupAlignEquals || groupKeys && group == null) { return key.length(); }
