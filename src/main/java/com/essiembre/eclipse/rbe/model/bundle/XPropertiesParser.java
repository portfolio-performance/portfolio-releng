...
import com.essiembre.eclipse.rbe.model.workbench.XRBEPreferences;

public final class XPropertiesParser
{
     ...
     * expected to match the documented structure of a properties file. The
     * returned bundle will have no <code>Locale</code> and no
    ...
    public static Bundle parse(String properties)
    {
        ...
        for (int i = 0; i < lines.length; i++)
        {
            ...
            if (equalPosition >= 1 && (isRegularLine || isCommentedLine))
            {
                ...
                if (XRBEPreferences.getConvertEncodedToUnicode())
                {
                    key = XPropertiesParser.convertEncodedToUnicode(key);
                    value = XPropertiesParser.convertEncodedToUnicode(value);
                }
                else
                {
                ...
                if (aChar == 'u' && x + 4 <= len)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        switch (aChar)
                        {
                            ...
                            default:
                                value = aChar;
                                System.err.println("error.init.badencoding: " + str);
                        }
                    }
